...
/* Demo conversation for Grade 11 Physics */
const DEMO_MESSAGES = [
  {
    id:1, role:"assistant", type:"text",
    content:`Hey there! 👋 I'm **Lumi**, your AI tutor — calibrated for **Grade 11 Physical Sciences (CAPS)**.\n\nI can see you're working with Newton's Laws. I'm here to help you *understand*, not just give answers. Whether it's a concept that's fuzzy, a calculation you're stuck on, or exam prep — let's crack it together! 💪\n\n**You can ask me anything, or try one of these:**`,
    suggestions:["Explain Newton's 2nd Law with a real example","Help me draw a Free Body Diagram","How do I solve friction problems?","What are Newton's 3rd Law action-reaction pairs?"]
  }
];
          ...
          {msg.suggestions?.length > 0 && (
            <div className="suggestions">
              {msg.suggestions.map((s, i) => (
                <span key={i} className="sug-btn" data-suggestion={s}>{s}</span>
              ))}
            </div>
          )}
        </div>
...
const DEMO_QS = [
  { q: "Q1b: Calculate the acceleration of a 5 kg block pushed by 20 N net force on a frictionless surface.", tag: "Calculation" },
  { q: "Q1e: Explain Newton's 3rd Law — what happens when a rocket expels gas downward?", tag: "Essay" },
  { q: "Q1d: Calculate max static friction for a 10 kg object. μs = 0.4, g = 10 m/s²", tag: "Calculation" },
  { q: "Q2b: A sound wave travels at 340 m/s with wavelength 0.68 m. Find frequency.", tag: "Calculation" },
  { q: "Q3b: A 10 Ω resistor on a 12 V battery. Calculate the current flowing.", tag: "Ohm's Law" },
  { q: "Explain why normal force ≠ weight on an inclined plane.", tag: "Concept" },
  { q: "What is the difference between static and kinetic friction?", tag: "Concept" },
  { q: "I don't understand what a Free Body Diagram is. Can you help?", tag: "FBD" },
];
...
function TutorTab() {
  ...
  function handleGradeChange(g) {
    ...
    setMessages([{
      ...
      time: nowTime(),
      suggestions: g === "Grade 11" ? ["Help me with Newton's Laws","Explain a formula","What will come in the exam?","Draw a Free Body Diagram"] : undefined
    }]);
  }
      ...
      <div className="tutor-shell">
        ...
        <div className="tutor-sidebar">
          ...
          <div className="demo-section">
            <div className="ds-label">📋 Demo Questions (Gr 11 Physics)</div>
            {DEMO_QS.map((dq, i) => (
              <div key={i} className="demo-q" onClick={() => { setInput(dq.q); textareaRef.current?.focus(); }}>
                <div className="demo-q-num">{dq.tag}</div>
                {dq.q.length > 80 ? dq.q.slice(0, 80) + "…" : dq.q}
              </div>
            ))}
          </div>
        </div>
